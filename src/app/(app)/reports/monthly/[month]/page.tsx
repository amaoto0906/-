...
export default async function MonthlyReportPage({
  ...
  return (
    <div>
      <PageHeader
        title={`月報: ${params.month}`}
        description={`${toDateKey(from)} 〜 ${toDateKey(to)}`}
        action={{
          href: `/api/reports/monthly/pdf?month=${params.month}`,
          label: "📄 PDFダウンロード",
        }}
      />
      <div className="p-8 max-w-4xl">
        <div className="card p-8 space-y-6">
          <header className="text-center border-b pb-4">
            <h2 className="text-2xl font-bold">養魚場 月次報告書</h2>
            <p className="text-slate-500 mt-1">対象月: {params.month}</p>
          </header>
